...
def compare(model_a, model_b, output, top_k, top_n, token):
    ...
    t0 = time.time()
    path_a = resolve_model(model_a, token=token)
    path_b = resolve_model(model_b, token=token)
